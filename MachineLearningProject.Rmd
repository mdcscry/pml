...
Cross validation was performed using two different methods.  I bootstrapped the Random Forest and CV'd the boosted model.
...
#SAMPLE for speed
fittr1<-fittr2[sample(nrow(fittr2),10000),] #10000
...
####RF Model
train_control <- trainControl(method="boot",number=30)
modelFit <- train(Classe ~ .,data=training,method="parRF",prox=TRUE
...
modelFit2 <- train(Classe ~ ., method="gbm",data=training,trControl = fitControl,tuneGrid = gbmGrid,n.minobsinnode = c(5),verbose=FALSE)
modelFit2
